...
- [ ] **Tarea O.11 (Matriz crítica multinavegador) — APROBADA LOCALMENTE, PENDIENTE DE CI REMOTO**: La auditoría admin/trabajador de escritorio y móvil corre de forma aislada en Chromium, Firefox y WebKit; el gate funcional completo permanece en Chromium. Evidencia: matriz 15/15 sin reintentos, `npm.cmd run verify` con typechecks, lint, build, SheetJS, Worker 6/6 y Playwright 16/16, y `npm.cmd audit --audit-level=high` con 0 vulnerabilidades. CI instala los tres motores y ejecuta ambos gates con permisos `contents: read`; falta confirmar la corrida remota después de un push explícitamente autorizado a `main`.

## 📊 Estado General del Proyecto
- **Fase Actual**: Fase 7 — Payroll, QA de datos y jornadas administrativas (COMPLETADA Y DESPLEGADA)
- **Mantenimiento actual**: Tarea O.11 — matriz crítica multinavegador (APROBADA LOCALMENTE, PENDIENTE DE CI REMOTO).
