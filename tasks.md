...
- [x] **Tarea O.10 (Auditoría integral de legibilidad admin/trabajador) — DESPLEGADA**: Revisadas ambas experiencias en escritorio y móvil con medición del DOM. Se corrigieron avisos semánticos ilegibles (baseline mínimo 1.07:1), placeholders (2.33:1), acciones (2.10:1), etiquetas, selectores y diálogos sin nombre accesible. Evidencia local: 15/15 repeticiones críticas, Worker 6/6, Playwright 16/16 y `npm audit` con 0 vulnerabilidades. Producción automática observada tras el push solicitado: commit `7334a64`, GitHub Actions `Verify #17` (`33220506253`) aprobado y checks Vercel `fieldhours`/`field-hours` exitosos. El alias público sirve `index-By-75Z5C.js` con las firmas accesibles nuevas; frontend y health directo/proxy HTTP 200, rutas protegidas 401, CSP/HSTS/`nosniff` presentes. Sin cambios de Worker, migraciones ni escrituras productivas. Rollback en `docs/RELEASE-2026-08-28-legibility.md`.
- [x] **Tarea O.11 (Matriz crítica multinavegador) — ACTIVA EN CI, PRODUCCIÓN REVALIDADA**: La auditoría admin/trabajador de escritorio y móvil corre de forma aislada en Chromium, Firefox y WebKit; el gate funcional completo permanece en Chromium. Evidencia local: matriz 15/15 sin reintentos, `npm.cmd run verify` con typechecks, lint, build, SheetJS, Worker 6/6 y Playwright 16/16, y `npm.cmd audit --audit-level=high` con 0 vulnerabilidades. Evidencia remota: commit `dc7575a`, GitHub Actions `Verify #18` (`33222205776`) y su paso multinavegador aprobados; checks Vercel exitosos. Smoke de solo lectura: frontend/bundle y health directo/proxy 200, rutas protegidas 401, CSP/HSTS/`nosniff` presentes. Sin cambios de aplicación, Worker, D1 ni escrituras productivas.
- [x] **Tarea O.12 (Validación productiva de Google OAuth) — COMPLETADA**: Confirmados los nombres de los secretos requeridos sin leer valores, tablas D1 `0004`, callback productivo y `mode=invalid` HTTP 400 directo/proxy antes de crear estado. Una identidad no vinculada recorrió Google y creó una solicitud `access`; el administrador local la rechazó desde la UI y D1 confirmó pendientes 0 y evento de auditoría. Una identidad ya vinculada completó el login, obtuvo interfaz de trabajador con mensaje de éxito y cero controles administrativos; después se cerró la sesión. Las identidades permanecieron en 2, sin usuarios nuevos, estados activos ni pendientes. Sin cambios de código, secretos, Worker, migraciones o despliegue. Evidencia: `docs/RELEASE-2026-08-28-google-oauth-production.md`.
- [x] **Tarea O.13 (Alertas y recuperación D1) — DESPLEGADA**: Monitor HTTP con reintentos después de cada `Verify` exitoso, cada 30 minutos y manualmente; incidencia GitHub deduplicada y autocierre. El ensayo restaura un export D1 sintético en una segunda D1 local aislada. Evidencia local: monitor 2/2, D1 con 50 objetos/13 contadores idénticos y cero violaciones FK, `npm ci`, gate completo, Playwright 16/16 y 0 vulnerabilidades. Evidencia remota: commits `83177aa`/`5a469b3`, Verify `33228132974`/`33229050228`, Production health `33229189380` (job `99038776374`) y ambos checks Vercel exitosos; smoke productivo 5/5 y cero incidencias abiertas. No hubo cambios de Worker, migraciones, secretos ni escrituras productivas. Runbook: `docs/OPERATIONS.md`; release/rollback: `docs/RELEASE-2026-08-28-operations.md`.
- [x] **Tarea O.14 (Pepper de contraseñas) — DESPLEGADA Y CERRADA**: Eliminado el fallback fijo y retirado `PASSWORD_PEPPER_LEGACY` con autorización explícita, conservando `PASSWORD_PEPPER_CURRENT`. Gate previo: 3 cuentas activas, 1 hash `v2$`, 2 legados con Google, `legacy_admins = 0`, `legacy_without_google = 0`, cero escrituras y monitor 5/5. Cloudflare desplegó la versión de cambio de secreto `1f34fd8f-8449-4a0e-aa25-8da6e7c480e6`; la lista posterior confirmó ausencia del binding legado sin leer valores. Monitor posterior 5/5 y login sintético directo/proxy HTTP 401 `INVALID_CREDENTIALS`; conteos D1 sin cambios. Los intentos sintéticos solo registraron rate-limiting normal, sin usuarios ni sesiones. Rollback a versión compatible `e4fb038c-ca09-409b-8158-d3b1a569f2e4`; usuarios legados conservan Google/restablecimiento. ADR: `docs/adr/ADR-001-password-pepper-versioning.md`; release: `docs/RELEASE-2026-08-28-password-pepper.md`.
- [x] **Tarea O.15 (Paridad de seguridad en frontend staging) — DESPLEGADA**: Pages usa Advanced Mode, por lo que las cabeceras se aplican directamente en `public/_worker.js` y una prueba de contrato exige paridad con Vercel sin alterar `/api/*`. La primera publicación `8021b026` corrigió CSP pero reveló HSTS ausente; la segunda vuelta agregó el valor productivo exacto. Gate: typechecks, lint, build, SheetJS, Worker 14/14, operaciones/Pages 4/4, recuperación D1, Playwright 16/16 y audit 0. Deployment final `9402ae49-bf50-4bca-b8ce-ea47c60da701`; alias e URL inmutable aprobaron monitor 5/5 con HTTP 200/401, CSP, HSTS y `nosniff`. Sin cambios de producción, Worker backend, D1 o secretos. Release/rollback: `docs/RELEASE-2026-08-28-staging-csp.md`.
- [x] **Tarea O.16 (Fusión de cuenta trabajadora duplicada) — DESPLEGADA**: Se conservó activa la identidad Google de Flávia, se transfirieron los 3 turnos completos de la cuenta local y se desactivó el duplicado sin borrar eventos append-only ni registros financieros. Las consultas de nómina excluyen usuarios desactivados. Commit `17c1916`, Verify `33232730861`, ambos checks Vercel y Worker `e4fb038c-ca09-409b-8158-d3b1a569f2e4` aprobados; monitor 5/5. D1 quedó con una sola Flávia activa, 6 turnos/22 eventos en Gmail, 12 autores históricos preservados, una auditoría de fusión y cero violaciones FK. La UI administrativa confirmó una sola fila de nómina y las 6 jornadas consolidadas. Release/rollback: `docs/RELEASE-2026-08-28-worker-account-merge.md`.
...
## 📌 Fase 6: Inicio de sesión con Google y aprobación administrativa *(Completada y desplegada)*
