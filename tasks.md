...
- [ ] **Tarea O.14 (Pepper de contraseñas) — WORKER PRODUCCIÓN DESPLEGADO, RETIRO LEGADO PENDIENTE**: Eliminado el fallback fijo; hashes nuevos `v2$` con `PASSWORD_PEPPER_CURRENT`, puente temporal `PASSWORD_PEPPER_LEGACY` y rehash automático antes de sesión. Seguridad corrigió además la falta de ignore para `.dev.vars`. Evidencia local: Worker 13/13, Playwright 16/16, gate funcional, audit 0 y dry-run. Staging `1981cf0f-965a-4942-ab3d-fc1ec0c691ae`: health directo/proxy 200, límites 401 y login sintético `401 INVALID_CREDENTIALS`. Producción autorizada y desplegada como Worker `14f4748b-88a4-48db-aefe-174d6fe77dca`: monitor 5/5; el login administrativo dejó 1 hash `v2$`, 0 administradores legados y 1 evento de auditoría, sin escrituras de la consulta. Queda 1 cuenta local sin Google en legado; no retirar `PASSWORD_PEPPER_LEGACY` hasta su login o restablecimiento. ADR/rollback: `docs/adr/ADR-001-password-pepper-versioning.md`; release: `docs/RELEASE-2026-08-28-password-pepper.md`.
...
- [ ] **Tarea O.16 (Fusión de cuenta trabajadora duplicada) — LISTA PARA PRODUCCIÓN**: Conservar la identidad Google de Flávia, transferirle 3 turnos completos de la cuenta local y desactivar el duplicado sin borrar eventos append-only ni registros financieros. Se corrigieron tres consultas de nómina para excluir usuarios desactivados. Diagnóstico remoto sin colisiones ni escrituras; Worker 14/14, gate completo, Playwright 16/16, audit 0, dry-run y ensayo D1 fusión/rollback aprobados. Requiere autorización explícita para push/despliegue del Worker y transacción D1. Release/rollback: `docs/RELEASE-2026-08-28-worker-account-merge.md`.

## 📊 Estado General del Proyecto
- **Fase Actual**: Fase 7 — Payroll, QA de datos y jornadas administrativas (COMPLETADA Y DESPLEGADA)
- **Mantenimiento actual**: Tareas O.14/O.16 — fusión de la última cuenta local preparada; después de validarla podrá reevaluarse el retiro de `PASSWORD_PEPPER_LEGACY`.
