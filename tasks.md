...
- [x] **Tarea O.10 (Auditoría integral de legibilidad admin/trabajador) — APROBADA, NO DESPLEGADA**: Revisadas ambas experiencias en escritorio y móvil con medición del DOM. Se corrigieron avisos semánticos ilegibles (baseline mínimo 1.07:1), placeholders (2.33:1), acciones (2.10:1), etiquetas, selectores y diálogos sin nombre accesible. La regresión permanente valida contraste WCAG AA, placeholders, foco visible, nombres de controles/diálogos y overflow en vistas admin/trabajador, modales y error de nómina. Evidencia: 15/15 repeticiones críticas sin reintentos, gate con Worker 6/6 y Playwright 16/16, capturas revisadas, `git diff --check` limpio y `npm audit` con 0 vulnerabilidades.

## 📊 Estado General del Proyecto
- **Fase Actual**: Fase 7 — Payroll, QA de datos y jornadas administrativas (COMPLETADA Y DESPLEGADA)
- **Mantenimiento actual**: Tarea O.10 — auditoría integral de legibilidad (APROBADA, NO DESPLEGADA).
