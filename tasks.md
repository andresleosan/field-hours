...
- [~] **Tarea O.6 (Defensa contra abuso de operaciones sensibles)**: Implementado límite por organización/administrador para revelado de perfiles (10/15 min) y preparación de Salary Advice (30/15 min), con `Retry-After`, claves SHA-256 namespaced y variables diferenciadas por entorno; política en `docs/RATE_LIMITING.md`. Typecheck del Worker correcto. Falta cerrar con prueba HTTP local de `429` y recuperación tras ventana antes de desplegar.
