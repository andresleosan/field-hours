...
- [ ] **Tarea O.14 (Pepper de contraseñas) — HALLAZGO ALTO**: El secreto `PASSWORD_PEPPER` no existe en producción y `cloudflare/src/auth.ts` usa un fallback fijo versionado. Diseñar y verificar una migración compatible con la cuenta administrativa local antes de cualquier despliegue del endurecimiento; no rotar el pepper sin un camino probado de acceso/rehash.

## 📊 Estado General del Proyecto
- **Fase Actual**: Fase 7 — Payroll, QA de datos y jornadas administrativas (COMPLETADA Y DESPLEGADA)
- **Mantenimiento actual**: Tarea O.13 — alertas y práctica controlada de recuperación D1 (DESPLEGADA). Próxima prioridad: O.14, endurecimiento seguro del pepper de contraseñas sin perder acceso administrativo.
