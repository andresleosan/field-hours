...
> Estado: el perfil, configuración, preview y flujo de revisión/aprobación de nómina están desplegados en Worker/Vercel con D1 `0008`; no se ejecutan transferencias automáticas. El payslip final sigue pendiente.
...
- [x] **Tarea O.2 (QA E2E reproducible)**: Playwright migrado a `@playwright/test`, runner multiplataforma con servidor Vite controlado, scripts `test:e2e*` y suite `e2e/` aislada de producción. Valida submit/aprobación/bloqueo y solicitud de cambios de nómina, separación de roles, CSRF fail-closed, proyecto creado por trabajador, dos breaks de distinta duración, dos shifts diarios, GPS sin foto, totales/historial y viewport móvil sin overflow. Evidencia: listado de 5 pruebas y suite 5/5 en verde; reporte en `qa/TEST_REPORT.md` y HTML local en `qa/reports/playwright/`.
- [x] **Tarea O.3 (Dependencias y seguridad)**: Remediadas las 12 vulnerabilidades altas y 1 moderada iniciales. React Router 7.18.2, Vite 8.2.2, plugin React SWC 4.3.3, PostCSS 8.5.26, SheetJS CE 0.20.3 oficial y transitivas compatibles; Playwright movido a `devDependencies` y configuración Vite migrada a Rolldown. Evidencia: `npm audit` en 0 vulnerabilidades, árbol válido, smoke SheetJS, typechecks, lint sin errores, build y E2E 5/5 aprobados. Reporte: `qa/DEPENDENCY_SECURITY_REPORT.md`.
- [ ] **Tarea O.4 (Gate CI y datos de navegadores)**: Crear un comando único de verificación y un pipeline CI que ejecute typechecks, lint, build, smoke SheetJS, E2E y audit; actualizar de forma controlada `caniuse-lite`/Browserslist para eliminar la advertencia de datos desactualizados. Criterio de cierre: pipeline reproducible en verde, sin secretos ni producción, y política documentada de actualización periódica.
- [ ] **Tarea O.5 (Higiene del build y paquete Vercel)**: Reducir el paquete de despliegue (actualmente 6,8 MB/411 archivos), excluir documentación y artefactos internos que no necesita el build, y resolver las advertencias de supply chain de Vercel (`glob@10.5.0` vía Tailwind/Sucrase y scripts de instalación de SWC/esbuild). Criterio de cierre: build frío reproducible, dependencias auditadas, scripts permitidos de forma explícita y upload limitado a archivos necesarios.
- [ ] **Tarea O.6 (Defensa contra abuso de operaciones sensibles)**: Definir e implementar un límite por administrador para las rutas que descifran perfiles o preparan Salary Advice, manteniendo reimpresiones legítimas y auditoría. Criterio de cierre: política documentada, respuesta `429` comprobada sin registrar referencias/importe, prueba de recuperación tras la ventana y configuración diferenciada para local/staging/producción.
