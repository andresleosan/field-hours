...
- [ ] **Tarea O.14 (Pepper de contraseñas) — STAGING VALIDADO, PRODUCCIÓN PENDIENTE**: Eliminado el fallback fijo; hashes nuevos `v2$` con `PASSWORD_PEPPER_CURRENT`, puente temporal `PASSWORD_PEPPER_LEGACY` y rehash automático antes de sesión. Seguridad corrigió además la falta de ignore para `.dev.vars`. Evidencia local: Worker 13/13, Playwright 16/16, gate funcional, audit 0 y dry-run. Staging `1981cf0f-965a-4942-ab3d-fc1ec0c691ae`: health directo/proxy 200, límites 401 y login sintético `401 INVALID_CREDENTIALS`; D1 agregado 1 legado, 0 admin y 0 local sin Google, sin escrituras. Producción sigue sin peppers y monitor 5/5; línea base 4 legados, 1 admin y 2 sin Google. ADR/rollback: `docs/adr/ADR-001-password-pepper-versioning.md`; release: `docs/RELEASE-2026-08-28-password-pepper.md`.
...
- **Mantenimiento actual**: Tarea O.14 — staging validado; producción pendiente de commit/CI y confirmación explícita separada.
