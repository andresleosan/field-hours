...
- [ ] **Tarea O.14 (Pepper de contraseñas) — WORKER PRODUCCIÓN DESPLEGADO, RETIRO LEGADO PENDIENTE**: Eliminado el fallback fijo; hashes nuevos `v2$` con `PASSWORD_PEPPER_CURRENT`, puente temporal `PASSWORD_PEPPER_LEGACY` y rehash automático antes de sesión. Seguridad corrigió además la falta de ignore para `.dev.vars`. Evidencia local: Worker 13/13, Playwright 16/16, gate funcional, audit 0 y dry-run. Staging `1981cf0f-965a-4942-ab3d-fc1ec0c691ae`: health directo/proxy 200, límites 401 y login sintético `401 INVALID_CREDENTIALS`. Producción autorizada y desplegada inicialmente como Worker `14f4748b-88a4-48db-aefe-174d6fe77dca`: monitor 5/5; el login administrativo dejó 1 hash `v2$` y 0 administradores legados. Tras O.16 hay 0 cuentas locales activas sin Google que dependan del hash legado; el retiro de `PASSWORD_PEPPER_LEGACY` es técnicamente elegible, pero sigue pendiente de autorización explícita por ser una eliminación de secreto productivo. ADR/rollback: `docs/adr/ADR-001-password-pepper-versioning.md`; release: `docs/RELEASE-2026-08-28-password-pepper.md`.
- [ ] **Tarea O.15 (Paridad de seguridad en frontend staging) — HALLAZGO MEDIO**: `https://field-hours-staging.pages.dev` responde 200 pero no incluye la CSP productiva exigida por el monitor; los cuatro contratos Worker/proxy pasan. Revisar `_headers`/configuración Pages sin relajar el contrato de producción.
...
- **Mantenimiento actual**: Tareas O.14/O.15 — el retiro de `PASSWORD_PEPPER_LEGACY` ya es técnicamente elegible, pero requiere autorización explícita; la paridad CSP de staging sigue pendiente.
