...
- [ ] **Tarea 6.5 (Configuración/QA)**: Configurar OAuth Client ID, secretos y redirect URI en Cloudflare; ejecutar typecheck/build y prueba real de login en staging. Evidencia: se creó el Worker `field-hours-api-staging`, la D1 remota `field-hours-staging` (`7b945f18-96c5-4535-a117-7156474e3ca2`) y se aplicaron las migraciones `0001` a `0009`; el frontend público está en `https://field-hours-staging.pages.dev`; `/api/health` responde `200` directamente y a través de Pages; typecheck/build y la suite E2E existente pasan. Pendiente: cargar `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` y `PAYROLL_ENCRYPTION_KEY` sólo en staging, registrar en Google Cloud el callback `https://field-hours-staging.pages.dev/api/auth/google/callback` y completar un login real con una cuenta de prueba.
